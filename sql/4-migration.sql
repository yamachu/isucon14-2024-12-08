...
DROP TABLE IF EXISTS latest_rides;
...
SELECT r.chair_id, r.id, r.user_id
FROM rides r
JOIN (
  SELECT chair_id, MAX(updated_at) AS max_updated_at
  FROM rides
  GROUP BY chair_id
) latest_rides_per_chair
ON r.chair_id = latest_rides_per_chair.chair_id
AND r.updated_at = latest_rides_per_chair.max_updated_at
ON DUPLICATE KEY UPDATE
  ride_id = VALUES(ride_id),
